...
def forward_frame(interfaceDest, data, length, vlan_id, vlan_table, source_interface_type):
    ...
    if interface_type == "access" and source_interface_type == "trunk":
    ...
    while True:
        ...
        if is_unicast:
            if dest_mac in mac_table:
                forward_frame(mac_table[dest_mac], data, length, vlan_id, vlan_table, interface_type)
            else:
                for o in interfaces:
                    if o != interface:
                        forward_frame(o, data, length, vlan_id, vlan_table, interface_type)
        else:
            for o in interfaces:
                if o != interface:
                    forward_frame(o, data, length, vlan_id, vlan_table, interface_type)
